...
public:
    ...
    int64_t GetButterflyBalance() const;
    ...
    int64_t GetButterflyDebit(const CTxIn& txin) const;
    int64_t GetButterflyCredit(const CTxOut& txout) const;
    ...
    int64_t GetCredit(const CTransaction& tx) const
    {
        ...
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nCredit += GetButterflyCredit(txout);
            } else
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nBFC, int64_t& nButterfly) const
    {
        nBFC = 0;
        nButterfly = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nButterfly += GetButterflyCredit(txout);
            } else
                nBFC += GetCredit(txout);
            if (!MoneyRange(nBFC)
                || !MoneyRange(nButterfly))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
        return true;
    };
...
public:
    ...
    mutable bool fAvailableButterflyCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableButterflyCreditCached;
    
    mutable int64_t nCredBFCCached;
    mutable int64_t nCredButterflyCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableButterflyCreditCached = false;
        
        nCredBFCCached = 0;
        nCredButterflyCached = 0;
        ...
        nAvailableButterflyCreditCached = 0;
        nChangeCached = 0;
        nOrderPos = -1;
    }
    ...
    bool GetCredit(int64_t& nCredBFC, int64_t& nCredButterfly, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredBFC = nCredButterfly = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredBFCCached = 0;
            nCredButterflyCached = 0;
            pwallet->GetCredit(*this, nCredBFCCached, nCredButterflyCached);
            fCreditSplitCached = true;
        };
        
        nCredBFC = nCredBFCCached;
        nCredButterfly = nCredButterflyCached;
        return true;
    }
    ...
    int64_t GetAvailableButterflyCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableButterflyCreditCached)
            return nAvailableButterflyCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableButterflyCredit() : value out of range");
            };
        };
        
        nAvailableButterflyCreditCached = nCredit;
        fAvailableButterflyCreditCached = true;
        return nCredit;
    };
