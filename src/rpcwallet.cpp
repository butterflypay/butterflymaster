...
Value getinfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("butterflybalance", ValueFromAmount(pwalletMain->GetButterflyBalance())));
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendtoaddress <butterflycoinaddress> <amount> [comment] [comment-to] [narration]\n" // Exchanges use the comments internally...
            "sendtoaddress <butterflycoinaddress> <amount> [narration]\n"
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            "sendfrom <fromaccount> <tobutterflycoinaddress> <amount> [minconf=1] [comment] [comment-to] [narration] \n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new ButterflyCoin stealth address for receiving payments anonymously."
            + HelpRequiringPassphrase());
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    if (!sxAddr.SetEncoded(sEncoded))
        throw runtime_error("Invalid ButterflyCoin stealth address.");
...
Value sendbfctoanon(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendbfctoanon <stealth_address> <amount> [narration] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
...
Value sendanontobfc(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendanontobfc <stealth_address> <amount> <ring_size> [narration] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
