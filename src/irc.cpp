...
void ThreadIRCSeed()
{
    ...
    for(;;)
    {
        ...
        if (fTestNet)
        {
            Send(hSocket, "JOIN #bfcoinTEST\r");
            Send(hSocket, "WHO #bfcoinTEST\r");
        } else
        {
            ...
            Send(hSocket, strprintf("JOIN #bfcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #bfcoin%02d\r", channel_number).c_str());
        };
