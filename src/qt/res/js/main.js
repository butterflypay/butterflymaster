...
var unit = {
    ...
    setType: function(type) {
        ...
        switch(type) {
            ...
            case 2:
                this.name = "uBFC",
                this.display = "&micro;BFC";
                break;

            case 3:
                this.name    = "sBFC",
                this.display = "Butterflyshi";
                break;

            default:
                this.name = this.display = "BFC";
        }
...
var overviewPage = {
    init: function() {
        ...
        this.butterflyBal = $("#butterflyBal"),
    ...
    updateBalance: function(balance, butterflyBal, stake, unconfirmed, immature) {
        if(balance == undefined)
            balance     = this.balance    .data("orig"),
            butterflyBal   = this.butterflyBal  .data("orig"),
            ...
            this.butterflyBal  .data("orig", butterflyBal),
        ...
        this.formatValue("butterflyBal",   butterflyBal);
    ...
    $("#recipients").append((
        ...
        +  '<div id="recipient[count]" class="recipient"> \
            <div class="flex-right"> \
                ...
                <input id="pay_to[count]" class="pay_to input_box" title="The address to send the payment to  (e.g. ButterflywrppUwwNUo1GCRDTibz)" placeholder="输入一个你将付款的蝴蝶币地址 (例如 ButterflywrppUwwNUo1GCRDTibz)" maxlength="128" oninput="base58.check(this);" onchange="$(\'#label[count]\').val(bridge.getAddressLabel(this.value));"/> \
            ...
            <div class="flex-right"> \
                ...
                <input id="label[count]" class="label input_box" title="Enter a label for this address to add it to your address book" placeholder="输入一个标签，这个地址将其添加到您的地址簿" maxlength="128"/> \
            </div> \
            <div class="flex-right"> \
                <label for="narration[count]" class="recipient">信息旁白:</label> \
                <input id="narration[count]" class="narration input_box" title="Enter a short note to send with payment (max 24 characters)" placeholder="输入一个信息发送与支付（最多24个字符）" maxlength="24" /> \
            </div> \
            <div class="flex-left"> \
                ...
                <select id="unit[count]" class="unit button is-inverse has-fixed-icon"  style="margin-left:10px; height:43px; width:100px;" onchange="unit.format(\'#amount[count]\', $(this).val());"> \
                    <option value="0" title="Butterfly"                    ' + (unit.type == 0 ? "selected" : "") + '>BFC</option> \
                    <option value="1" title="Milli-Butterfly (1 / 1000)"   ' + (unit.type == 1 ? "selected" : "") + '>mBFC</option> \
                    <option value="2" title="Micro-Butterfly (1 / 1000000)"' + (unit.type == 2 ? "selected" : "") + '>&micro;BFC</option> \
                    <option value="3" title="Butterflyshi (1 / 100000000)" ' + (unit.type == 3 ? "selected" : "") + '>Butterflyshi</option> \
                </select> \
            </div> \
