...
WalletModel::WalletModel(CWallet *wallet, OptionsModel *optionsModel, QObject *parent) :
    ...
    cachedBalance(0), cachedButterflyBal(0), cachedStake(0), cachedUnconfirmedBalance(0), cachedImmatureBalance(0),
...
qint64 WalletModel::getButterflyBalance() const
{
    return wallet->GetButterflyBalance();
}
...
void WalletModel::checkBalanceChanged()
{
    ...
    qint64 newButterflyBal = getButterflyBalance();
    qint64 newStake = getStake();
    qint64 newUnconfirmedBalance = getUnconfirmedBalance();
    qint64 newImmatureBalance = getImmatureBalance();

    if (cachedBalance != newBalance
        || cachedButterflyBal != newButterflyBal
        ...
        || cachedImmatureBalance != newImmatureBalance)
    {
        ...
        cachedUnconfirmedBalance = newUnconfirmedBalance;
        cachedImmatureBalance = newImmatureBalance;
        emit balanceChanged(newBalance, newButterflyBal, newStake, newUnconfirmedBalance, newImmatureBalance);
    }
}
...
WalletModel::SendCoinsReturn WalletModel::sendCoinsAnon(const QList<SendCoinsRecipient> &recipients, const CCoinControl *coinControl)
{
    ...
    if (vNodes.empty())
        return SendCoinsReturn(SCR_ErrorWithMsg, 0, QString::fromStdString("ButterflyCoin is not connected!"));
    ...
    {
        nBalance = wallet->GetButterflyBalance();
        if ((nTotalOut + nTransactionFee) > nBalance)
            return SendCoinsReturn(SCR_AmountWithFeeExceedsButterflyBalance, nTransactionFee);
    };
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            if (rcp.txnTypeInd == TXT_BFC_TO_BFC
                || rcp.txnTypeInd == TXT_ANON_TO_BFC)
            {
                // -- out bfc
                std::string sError;
