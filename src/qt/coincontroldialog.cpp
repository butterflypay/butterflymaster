...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog *dialog, ButterflyBridge *bridge)
{
    if (!model) return;
    ...
    } else
    {
        ...
        int nDisplayUnit = BitcoinUnits::BFC;
        ...
        l8->setStyleSheet((nChange > 0 && nChange < CENT) ? "color:red;" : ""); // Change < 0.01BFC
