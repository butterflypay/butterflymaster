...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx)
{
    ...
    if (wtx.mapValue.count("to") && !wtx.mapValue["to"].empty())
    {
        ...
        strHTML += "<b>" + tr("To") + ":</b> <a href='"+explorer + "address.asp?address=" + GUIUtil::HtmlEscape(strAddress) + "' target='_blank'>";
    ...
    if (wtx.IsCoinBase() && nCredit == 0)
    {
        ...
        if (wtx.IsInMainChain())
            strHTML += BitcoinUnits::formatWithUnit(BitcoinUnits::BFC, nUnmatured)+ " (" + tr("matures in %n more block(s)", "", wtx.GetBlocksToMaturity()) + ")";
        else
    ...
    if (nNet > 0)
    {
        ...
        // Credit
        //
        strHTML += "<b>" + tr("Credit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::BFC, nNet) + "<br>";
    } else
    {
        ...
        if (fAllFromMe)
        {
            ...
            BOOST_FOREACH(const CTxOut& txout, wtx.vout)
            {
                ...
                if (!wtx.mapValue.count("to") || wtx.mapValue["to"].empty())
                {
                    ...
                    {
                        strHTML += "<b>" + tr("To") + ":</b> <a href='"+explorer+"address.asp?address="+GUIUtil::HtmlEscape(CBitcoinAddress(address).ToString())+"' target='_blank'>";
                ...
                strHTML += "<b>" + tr("Debit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::BFC, -txout.nValue) + "<br>";
            }
            ...
            if (fAllToMe)
            {
                ...
                strHTML += "<b>" + tr("Debit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::BFC, -nValue) + "<br>";
                strHTML += "<b>" + tr("Credit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::BFC, nValue) + "<br>";
            }

            int64_t nTxFee = nDebit - wtx.GetValueOut();
            if (nTxFee > 0)
                strHTML += "<b>" + tr("Transaction fee") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::BFC, -nTxFee) + "<br>";
        } else
        {
            //
            ...
            BOOST_FOREACH(const CTxIn& txin, wtx.vin)
            {
                ...
                if (wallet->IsMine(txin))
                    strHTML += "<b>" + tr("Debit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::BFC, -wallet->GetDebit(txin)) + "<br>";
            };

            BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                if (wallet->IsMine(txout))
                    strHTML += "<b>" + tr("Credit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::BFC, wallet->GetCredit(txout)) + "<br>";
        }
    }

    strHTML += "<b>" + tr("Net amount") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::BFC, nNet, true) + "<br>";
    ...
    strHTML += "<b>" + tr("Transaction ID") + ":</b> <a href='"+explorer+"transaction.asp?txid=" + txid + "' target='_blank'>" + txid + "</a><br>";
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
            ...
            if (txdb.ReadDiskTx(prevout.hash, prev))
            {
                if (prevout.n < prev.vout.size())
                {
                    ...
                    {
                        strHTML +="<a href='"+explorer+"address.asp?address="+QString::fromStdString(CBitcoinAddress(address).ToString())+"' target='_blank'>";
                        if (wallet->mapAddressBook.count(address) && !wallet->mapAddressBook[address].empty())
                            strHTML += GUIUtil::HtmlEscape(wallet->mapAddressBook[address]) + " ";
                        strHTML += QString::fromStdString(CBitcoinAddress(address).ToString()) + "</a>";
                    }
                    strHTML = strHTML + " " + tr("Amount") + "=" + BitcoinUnits::formatWithUnit(BitcoinUnits::BFC, vout.nValue);
