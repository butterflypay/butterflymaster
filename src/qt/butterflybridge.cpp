...
#include "butterflybridge.moc"

ButterflyBridge::ButterflyBridge(ButterflyGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
ButterflyBridge::~ButterflyBridge()
{
...
void ButterflyBridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void ButterflyBridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void ButterflyBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void ButterflyBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void ButterflyBridge::paste() {
    emitPaste(QApplication::clipboard()->text());
}

// Options
void ButterflyBridge::populateOptions()
{
...
// Transactions
bool ButterflyBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void ButterflyBridge::clearRecipients()
{
    recipients.clear();
}

bool ButterflyBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    ...
    {
        int inputType; // 0 BFC, 1 Butterfly
        switch(rcp.txnTypeInd)
        {
            case TXT_BFC_TO_BFC:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BFC, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_BFC_TO_ANON:
                formatted.append(tr("<b>%1</b> to BUTTERFLY %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BFC, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b>, ring size %2 to BUTTERFLY %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BFC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_BFC:
                formatted.append(tr("<b>%1</b>, ring size %2 to BFC %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BFC, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BFC, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsButterflyBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your butterfly balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::BFC, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void ButterflyBridge::openAddressBook(bool sending)
{
...
void ButterflyBridge::openCoinControl()
{
...
void ButterflyBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void ButterflyBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap ButterflyBridge::listAnonOutputs()
{
...
void ButterflyBridge::populateTransactionTable()
{
...
void ButterflyBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void ButterflyBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString ButterflyBridge::transactionDetails(QString txid)
{
...
// Addresses
void ButterflyBridge::populateAddressTable()
{
...
void ButterflyBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void ButterflyBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString ButterflyBridge::newAddress(bool own)
{
...
QString ButterflyBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void ButterflyBridge::updateAddressLabel(QString address, QString label)
{
...
bool ButterflyBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool ButterflyBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void ButterflyBridge::appendMessages(QString messages, bool reset)
{
    emitMessages("[" + messages + "]", reset);
}

void ButterflyBridge::appendMessage(int row) {
...
void ButterflyBridge::populateMessageTable()
{
...
void ButterflyBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool ButterflyBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool ButterflyBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString ButterflyBridge::getPubKey(QString address, QString label)
{
    if(!label.isEmpty())
...
bool ButterflyBridge::setPubKey(QString address, QString pubkey)
{
...
bool ButterflyBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
    WalletModel::UnlockContext ctx(window->walletModel->requestUnlock());
...
void ButterflyBridge::connectSignals()
{
...
QVariantMap ButterflyBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap ButterflyBridge::listLatestBlocks()
{
...
QVariantMap ButterflyBridge::findBlock(QString searchID)
{
...
QVariantMap ButterflyBridge::blockDetails(QString blkHash)
{
...
QVariantMap ButterflyBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap ButterflyBridge::txnDetails(QString blkHash, QString txnHash)
{
