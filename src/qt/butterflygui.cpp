// Copyright (c) 2015 The ButterflyCoin developers
...
ButterflyGUI::ButterflyGUI(QWidget *parent):
    QMainWindow(parent),
    bridge(new ButterflyBridge(this)),
    clientModel(0),
    walletModel(0),
    messageModel(0),
    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("Butterfly") + " - " + tr("Client"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/butterflyw"));
    setWindowIcon(QIcon(":icons/butterfly"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/butterfly/index.html");
#else
    QFile html("/opt/butterfly/index.html");
#endif
...
void ButterflyGUI::pageLoaded(bool ok)
{
...
void ButterflyGUI::addJavascriptObjects()
{
    documentFrame->addToJavaScriptWindowObject("bridge", bridge);
}

void ButterflyGUI::urlClicked(const QUrl & link)
{
...
void ButterflyGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About ButterflyCoin"), this);
...
void ButterflyGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void ButterflyGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("ButterflyCoin") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/butterfly_testnet"));
            setWindowIcon(QIcon(":icons/butterfly_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/butterfly_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("ButterflyCoin client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/butterfly_testnet"));
...
void ButterflyGUI::setWalletModel(WalletModel *walletModel)
{
...
void ButterflyGUI::setMessageModel(MessageModel *messageModel)
{
...
void ButterflyGUI::createTrayIcon()
{
    QMenu *trayIconMenu;
#ifndef Q_OS_MAC
    ...
    trayIcon->setToolTip(tr("Butterfly client"));
    trayIcon->setIcon(QIcon(":/icons/butterfly"));
...
#ifndef Q_OS_MAC
void ButterflyGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void ButterflyGUI::aboutClicked()
{
    AboutDialog dlg;
...
void ButterflyGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("title", tr("%n active connection(s) to ButterflyCoin network", "", count));
}

void ButterflyGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void ButterflyGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void ButterflyGUI::changeEvent(QEvent *e)
{
...
void ButterflyGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void ButterflyGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::BFC, nFeeRequired));
...
void ButterflyGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void ButterflyGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void ButterflyGUI::gotoSignMessageTab(QString addr)
{
...
void ButterflyGUI::gotoVerifyMessageTab(QString addr)
{
...
void ButterflyGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void ButterflyGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void ButterflyGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void ButterflyGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
void ButterflyGUI::handleURI(QString strURI)
{
...
void ButterflyGUI::setEncryptionStatus(int status)
{
...
void ButterflyGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void ButterflyGUI::backupWallet()
{
...
void ButterflyGUI::changePassphrase()
{
...
void ButterflyGUI::unlockWallet()
{
    if(!walletModel)
        return;
...
void ButterflyGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void ButterflyGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void ButterflyGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void ButterflyGUI::updateStakingIcon()
{
...
void ButterflyGUI::detectShutdown()
{
